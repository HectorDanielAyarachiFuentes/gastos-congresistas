...
import { Home, AlertCircle, X, Users, ShieldAlert, ArrowDownAZ, ArrowUpAZ, TrendingUp, BarChart2, ChevronDown, ChevronUp, Filter } from 'lucide-react';
...
export default function LegislatorSelector({
  ...
  return (
    <div className="w-full h-full md:grid md:grid-cols-[300px_1fr] pointer-events-none md:pointer-events-auto">
      
      {/* 1. FILTERS (Left Sidebar on desktop, Main view on mobile 'list') */}
      <div className={`flex flex-col h-full bg-white border-r border-gray-200 transition-transform duration-300 absolute inset-0 z-20 md:relative md:z-0 md:translate-x-0 pointer-events-auto overflow-y-auto ${mobileView === 'list' ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b shrink-0">
          ...
          <button
            ...
            className="flex items-center justify-between w-full mt-3 py-1.5 px-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded text-sm font-semibold text-gray-700 transition-colors"
          >
          ...
          {filtersExpanded && (
            <div className="mt-3 space-y-2 text-sm animate-in fade-in slide-in-from-top-2 duration-200">
              <div>
        ...
        {/* Results attached at the bottom of filters */}
        <button 
          onClick={() => setResultsExpanded(!resultsExpanded)}
          className="w-full text-left border-t-8 border-gray-100 p-4 border-b shrink-0 sticky top-0 bg-white/95 backdrop-blur z-10 shadow-sm flex justify-between items-center hover:bg-gray-50 transition-colors"
        >
          <h2 className="font-bold text-gray-800">Resultados ({filteredAndSorted.length})</h2>
          {resultsExpanded ? <ChevronUp size={20} className="text-gray-500" /> : <ChevronDown size={20} className="text-gray-500" />}
        </button>
        {resultsExpanded && (
          <div className="selector-results-scroll flex-1 px-2 animate-in fade-in slide-in-from-top-2 duration-200">
            {filteredAndSorted.slice(0, 150).map((l: Legislator) => {
          ...
          {filteredAndSorted.length > 150 && (
            <div className="p-4 text-center text-xs text-gray-500 bg-gray-50 border-b border-gray-200">
              Mostrando 150 de {filteredAndSorted.length} resultados. Usa los filtros para refinar.
            </div>
          )}
        </div>
        )}
      ...
      {/* 2. CHART (Middle) */}
      <div className={`w-full h-full relative transition-transform duration-300 absolute inset-0 z-10 md:relative md:z-0 md:translate-x-0 pointer-events-auto bg-gray-100 ${mobileView === 'chart' ? 'translate-x-0' : 'translate-x-full'}`}>
        {children}
      </div>
    </div>
  );
